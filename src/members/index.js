...
import {
    ...
    SimpleForm, ImageInput, ImageField, RadioButtonGroupInput, DateInput, SaveButton,
    Toolbar, FormDataConsumer
} from 'react-admin';
import connect from './FormDataConsumer';
...
const PostBulkActions = props => (
    <BulkActions {...props} label="批量操作">
        <ResetViewsAction label="用户状态操作" alwaysOn/>
...
//=======区域列表二级联动
// const optionRenderer = choice => `${area_province.province_id} ${area_province.province_name}`;
...
export const MemberList = withStyles(listStyles)(({classes, ...props}) => (
    <List {...props} sort={{field: 'group_id', order: 'ASC'}} perPage={20} filters={<MemberFilter/>} title="成员查询"
          bulkActions={< PostBulkActions className={classes.block}/>}>
        <Datagrid>
...
export const MemberCreate = withStyles(createStyles)(({classes, ...props}) => (
    <Create {...props} title="成员添加">
        <SimpleForm redirect="false">
            ...
            <ImageInput source="avatar" label="头像上传" accept="image/*" placeholder={<p>点击上传头像</p>}
            ...
            <RadioButtonGroupInput source="sex" label="性别"
                                   choices={[{id: 'male', name: '男'}, {id: 'female', name: '女'}]}/>
            ...
            <ImageInput source="pic" label="照片上传" accept="image/*" placeholder={<p>点击上传照片</p>}
                        className={classes.stock}>
...
export const MemberEdit = withStyles(createStyles)(({classes, ...props}) => (
    <Edit {...props} title="成员编辑">
        <TabbedForm>
            ...
            <FormTab label="角色设置">
                <ReferenceInput label="角色设置" source="groups"                                   reference="groups">
                    <SelectInput optionText="reviser" />
                </ReferenceInput>
            </FormTab>
        </TabbedForm>
